...
if ($_POST['area'] == "static" ) {
	...
	$result = $db->query("SELECT p.id, p.tags, p.category, p.short_story, p.full_story, p.xfields, p.title, p.allow_br, e.news_id FROM " . PREFIX . "_post p LEFT JOIN " . PREFIX . "_post_extras e ON (p.id=e.news_id) LIMIT ".$startfrom.", ".$count_per_step);
...
# Auto Tags v1.1
		...
		$db->query( "UPDATE " . PREFIX . "_post SET tags='$tags', short_story='$short_story', full_story='$full_story', xfields='$filecontents' WHERE id='{$row['id']}'" );

// Auto Tags
		if ( $row['news_id'] ) {
			$_tags = explode( ",", $tags );
			foreach( $_tags as $_tag ) {
				$db->query( "INSERT INTO " . PREFIX . "_tags (news_id, tag) VALUES('{$row['id']}', '{$_tag}')" );
			}
		}
// Auto Tags
